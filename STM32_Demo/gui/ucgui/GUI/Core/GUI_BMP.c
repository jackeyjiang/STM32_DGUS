...
int GUI_BMP_DrawEx(GUI_GET_DATA_FUNC *pfGetData, void *p, int x0, int y0){
  ...
  U32 ClrUsed, Compression;
  int NumColors;
  int oneTimeLines = 0, NumBytes, DrawTimes;
  const U8 * pSrc;

  RetValue = (*pfGetData)(p, &pSrc, 54, 1);
  if(RetValue != 54)
    return 1;
  ...
  switch (BitCount) {
    ...
    case 1:   
        NumColors = 2;
        BytesPerLine = ((Width + 31) >> 5) << 2;
        break;
    case 4:   
        NumColors = 16; 
        BytesPerLine = (((Width << 2) + 31) >> 5) << 2;
        break;
    case 8:   
        NumColors = 256; 
        BytesPerLine = ((Width +  3) >> 2) << 2;
        break;
    case 24:  
        NumColors = 0; 
  ...
  oneTimeLines = GUI_DRAW_ALLOC_MEM / BytesPerLine;
  DrawTimes = Height + (oneTimeLines - 1) / oneTimeLines;
  if(oneTimeLines >= Height)
    oneTimeLines = Height;
  NumBytes = oneTimeLines * BytesPerLine;
  ...
  switch (BitCount) {
    ...
    case 8:
      ...
      {
        RetValue = (*pfGetData)(p, &pSrc, NumBytes, 0);
        if(RetValue != NumBytes)
          return 1;
        y0 -= oneTimeLines;
        Ret = _DrawBitmap_Pal(pSrc, x0, y0, Width, oneTimeLines, BitCount, NumColors);
        Height -= oneTimeLines;
        if(oneTimeLines >= Height)
          oneTimeLines = Height;
        pSrc += oneTimeLines * BytesPerLine;
        NumBytes = oneTimeLines * BytesPerLine;
      }
      break;
    case 24:
      for(i = 0; i < DrawTimes; i++)
      {
        ...
        if(RetValue != NumBytes)
          return 1;
        y0 -= oneTimeLines;
        Ret = _DrawBitmap_24bpp(pSrc, x0, y0, Width, oneTimeLines);
        Height -= oneTimeLines;
        if(oneTimeLines >= Height)
          oneTimeLines = Height;
        pSrc += oneTimeLines * BytesPerLine;
