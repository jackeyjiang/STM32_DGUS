       ...
       break;
		else 
		{
			...
			if(ReadBuf[0]=='n')
			{
				...
				{
					indexflag = Index-512+167+104; //³¤¶ÈÒª¼ÆËã120-26  256-120-26=
					res = f_lseek(&fsrc,indexflag);//Æ«ÒÆIndex+1021
...
void Sd_Write(char erro_flag,char takeout_flag)
{
   ...
   CmdLenght +=mem_copy01(&Send_Buf[CmdLenght],&PosDevNum[3],sizeof(PosDevNum)-3);       /*Pos»úÉè±¸ºÅ*/
   CmdLenght +=mem_copy01(&Send_Buf[CmdLenght],&PosTenantNum[3],sizeof(PosTenantNum)-3); /*pos»úÉÌ»§ºÅ*/
   CmdLenght +=mem_copy01(&Send_Buf[CmdLenght],&PosBatchNum[3],sizeof(PosBatchNum)-3);   /*posË¢¿¨Á÷Ë®ºÅ*/
   CmdLenght +=mem_copy01(&Send_Buf[CmdLenght],&PosUserNum[3],sizeof(PosUserNum)-3);     /*Ë¢¿¨ÕËºÅ*/   
