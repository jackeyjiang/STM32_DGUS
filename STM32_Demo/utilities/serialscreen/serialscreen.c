...
loop7:			if(!CloseCashSystem()){};//printf("cash system is erro5");  //关闭现金接受
			...
			case coins_key:  //按一次退一次
      {
				...
				if(VariableData[1]==0x01)
				{
					...
					//采用能发送几个是几个的方法是可行的，//CoinsTotoalMessageWriteToFlash.CoinTotoal = SendOutN_Coin(CoinsTotoalMessageWriteToFlash.CoinTotoal);
					cnt_t = SendOutN_Coin(CoinsTotoalMessageWriteToFlash.CoinTotoal);
					VariableChage(coins_in,cnt_t);//显示机内硬币数
